...
#   - только из частных 10.0.0.0/8 и 192.168.0.0/16: публичный диапазон (203.0.113.0/24) или 100.64.0.0/10
#     провайдера — это чьи-то настоящие адреса, и INPUT запер бы хост от них, а docker увёл бы к ним маршрут;
...
# Лежит ли подсеть (адрес числом и длина маски) целиком внутри диапазона, заданного строкой.
inside() {
  local start width
  start="$(number "${3%/*}")"
  width="${3#*/}"
  (($2 >= width && ($1 >> (32 - width)) == (start >> (32 - width))))
}
...
suitable() {
  ...
  if ! inside "$base" "$mask" 10.0.0.0/8 && ! inside "$base" "$mask" 192.168.0.0/16; then
    problems+=("не из частных 10.0.0.0/8 и 192.168.0.0/16: это чьи-то настоящие адреса")
  fi
