...
export const defaultProfile: ScreenProfile = { resolution: 1080, fps: 30, mode: 'text', automatic: true };
const bitrates = {
...
export function screenOptions(profile: ScreenProfile, codec: VideoCodec): TrackPublishOptions {
  const encoding = {
    maxBitrate: bitrates[profile.resolution][profile.fps] * 1000000,
    maxFramerate: profile.fps,
  };
  return {
    videoCodec: codec,
    backupCodec: codec === 'av1' || codec === 'vp9' ? { codec: 'vp8' } : false,
    screenShareEncoding: encoding,
    simulcast: true,
    degradationPreference: profile.mode === 'text' ? 'maintain-resolution' : 'maintain-framerate',
