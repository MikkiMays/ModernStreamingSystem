...
const CUES: Record<Cue, Note[]> = {
  ...
  */
  ...
  /*
    Ваш ход за карточным столом. Чистая квинта вверх — интервал, которого нет больше ни у
    одного сигнала: ни октавы микрофона, ни терций входа с ним не спутать даже вполуха.

    Он нужен именно потому, что на стол не всегда смотрят: человек отвернулся к разговору, а
    часы уже идут. Поэтому сигнал слышен, но короток — за раздачу он прозвучит несколько раз.
  */
  turn: [note(D5, 0, 0.09, 0.055), note(A5, 0.07, 0.14, 0.055)],
  // Вы забрали банк. Единственный сигнал из четырёх нот и единственный с трезвучием: событие
  // редкое, и звучать оно имеет право заметно.
  win: [
    note(C5, 0, 0.14, 0.06),
    note(E5, 0.06, 0.14, 0.06),
    note(G5, 0.12, 0.16, 0.06),
    note(C6, 0.18, 0.3, 0.065),
  ],
};
