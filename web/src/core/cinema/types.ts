 ...
 * и то, по чему решают, включать ли, — сайт, ступени качества, дорожки звука и субтитры. Плейлист по
 * ссылке — `kind: 'series'`: его серии приезжают страницей сериала (`series` службы), каждая своей
 * карточкой. Ключи — ровно те, что собирает служба (`wire.link_card`); необязательное — потому что
 * не каждый сайт это называет.
 */
export interface CinemaLinkItem extends CinemaItem {
  /** Сайт, откуда видео: `ok.ru`, `dzen.ru`. */
  site?: string | null;
  /** Ступени качества, которые покажет плеер, — лучшая первой: `4K`, `1080p`, `720p`. */
  qualities?: string[];
  /** Дорожки звука на выбор (у HLS — отдельные дорожки мастера). */
