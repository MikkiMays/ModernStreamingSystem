 ...
 * раньше — HLS ответил 403/410, DASH споткнулся, — адрес обновляется по отказу, но не
 * бесконечно: попытки считаются одним счётчиком на открытое видео, и у каждого пути свой предел.
 *
...
export function useSource(api: CinemaApi, watch: Watch, echo: Echo, renewed: () => void) {
  ...
  return { source, status, setStatus, error, setError, renew: renewSource, expired, dashFailed };
}
