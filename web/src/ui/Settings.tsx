    ...
    <section className="quality-section" aria-label={title}>
      ...
      <p className="form-footnote">
        {profile.automatic
          ...
          : `${profile.resolution}p · ${profile.fps} fps передаются как выбрано и не понижаются автоматически. Если канал не тянет, картинка замрёт вместо того, чтобы стать хуже. Выбранный уровень действует и на приём — то, что вы смотрите, тоже не ужимается под размер плитки; смена посреди разговора доедет до приёма при следующем входе.`}
      </p>
      {kind === 'camera' && (
        <p className="form-footnote">
