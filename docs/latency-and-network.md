...
**16.09.2026 это опасение подтвердилось прямым наблюдением, и оно оказалось сильнее ожидаемого.** В журнале `livekit-server` за сутки семь записей `status update delayed, possible deadlock` с задержками **32–48 секунд**; все приходятся на моменты сборки образов на этой же машине. То есть при нехватке ядер SFU встаёт не «чуть медленнее обрабатывает пакеты», а замирает целиком на десятки секунд. Тогда же замерена и сама пересылка: одно ядро вывозит 170–200 Мбит/с, десять участников в 1080p — четверть ядра и 114 МиБ, потери 0 %. Методика и полные таблицы — в [capacity.md](capacity.md).
...
В Cord сначала был устранён собственный запрос дополнительных 50 мс, а затем клиент стал просить нулевую буферизацию одинаково для аудио и видео. **Это оказалось ошибкой, и 16.09.2026 она снята.** Опасение из этого же абзаца — «при потере пакетов избыточно агрессивные настройки могут увеличить число фризов» — подтвердилось на практике: на канале со скачками задержки цель «ноль» заставляла NetEq выбрасывать звук, догоняя её (слышно как ускорение музыки), и оставляла буфер пустым к следующему всплеску (слышно как поломка). Теперь запас выбирается по классу дорожки, пути медиа и самочувствию декодера; разговор остаётся коротким, музыка получает право отстать. Подробно — в [resilience.md](resilience.md).
...
- Windows: компактная центральная главная, избранные комнаты, вариант для низкого окна, нативная анимация SplitView; один WebView2 сохраняется при скрытии панели. Общий web UI остаётся адаптивным.
...
- Запас буфера приёма выбирается адаптивно и по классу дорожки, а не задаётся числом: см. [resilience.md](resilience.md). Автоматическое восстановление и защита от отставания живого эфира сохранены; порог отставания теперь отсчитывается от запрошенного запаса, иначе он разрывал бы подписку из-за собственной настройки. Показ фактических метрик не заменяет измерение синхронизации звука и изображения.
