...
| ----------- | --------------- | ------------------ | ----------------------------------------------- |
| CPU         | 2 ядра          | 4 ядра             | На сборку при установке; дальше нагрузка меньше |
| RAM         | 4 ГБ            | 8 ГБ               | На 2 ГБ сборка веб-клиента может не завершиться |
| Диск        | 20 ГБ           | 40 ГБ              | Образы, база, временные файлы встреч            |
| Сеть        | 100 Мбит/с      | 1 Гбит/с           | Видео идёт через сервер — это главный расход    |
| IP          | публичный IPv4  | публичный IPv4     | За NAT без проброса портов работать не будет    |

Подойдёт почти любой VPS. Видео проходит **через сервер**, поэтому канал важнее процессора:
десять человек с включёнными камерами — это уже сотни мегабит исходящего трафика.
...
Список серверов на этом экране — закладки. Браузер может обратиться только к тому серверу,
который отдал ему страницу, поэтому выбор другого просто открывает его адрес.

### Cord для Windows

Скачать установщик, запустить. Приложение откроется на экране подключения: справа от
заголовка списка — **синий плюс**, у каждой строки — **шестерёнка**.
...
Сохранить сервер можно и не проверяя — например, пока он ещё собирается. Проверка отвечает
на вопрос и оставляет вас на месте: внизу слева зелёное «Подключено!» либо красное
«Не подключено» со стрелочкой, под которой написано, что именно не так.
