...
Ничего ставить не нужно. Открыть адрес сервера, ввести имя, войти.
Работает на компьютере и на телефоне.

### Cord для Windows

Скачать установщик, запустить, открыть **Настройки** и вписать адрес сервера.

В настройках теперь список серверов: свой, рабочий, друга — сколько нужно.
Переключение между ними — в том же окне.

| Поле | Что вписать |
| --- | --- |
| Сохранённые серверы | выбрать уже добавленный |
| Адрес сервера | `https://meet.example.com` или `https://203.0.113.10` |
| Название | любое, для себя: «Домашний», «Работа» |
| Забыть этот сервер | убрать из списка |
...
разрешения хранятся раздельно. Переключение на другой сервер не смешивает их.
