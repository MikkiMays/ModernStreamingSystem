...
Скрипт сам смотрит, что изменилось, и делает только нужное: пересобирает те службы, чьи
исходники поменялись, перевыпускает конфигурацию, если изменился её генератор, обновляет
образы базы и SFU, перезапускает и проверяет, что сервер отвечает. Прежние образы он
помечает **до** сборки и при неудаче возвращает сам — сервер не остаётся лежать. Нужен root:
плеер страниц кинозала работает только за стеной сети, а её проверяет iptables; без root скрипт
откажется сразу, ничего не тронув, если обновление касается плеера страниц.

```bash
./update.sh --check       # посмотреть, что изменилось, ничего не трогая
./update.sh --yes         # без вопросов: для cron
./update.sh --keep-calls  # не трогать SFU, чтобы идущие звонки не оборвались
./update.sh --force       # пересобрать всё, даже если ничего не менялось
```

Обновление перезапускает SFU, поэтому **идущие звонки оборвутся** — разговор начнут заново.
Если это дорого, `--keep-calls` пропустит всё, что трогает медиа, и в конце напомнит, что
осталось доделать. Совсем без перерыва, с закрытием входа и ожиданием конца встреч, — в
[deployment.md](deployment.md).

Раз в неделю по ночам, если хочется забыть про это:

```bash
(crontab -l 2>/dev/null; echo "0 5 * * 1 cd $PWD && ./update.sh --yes >> /var/log/cord-update.log 2>&1") | crontab -
```

Данные скрипт не удаляет ни при каком исходе: `docker compose down -v` в нём нет. `.env`
он тоже не трогает — секреты, пароль сервера и лимиты переживают обновление.
...
`<адрес>/download` работает сразу и ничего не требует: если своей копии сборки у сервера нет,
страница показывает последний выпуск проекта с GitHub, а если и он недоступен — постоянную
ссылку `releases/latest/download/Cord-Setup-x64.exe`, которая не требует ни одного запроса к
API. Кнопка «Скачать» не исчезает ни при каком раскладе; раньше на сервере без своей копии её
не было вовсе, что и было самым частым случаем.

Своя копия остаётся полезной: она быстрее, не зависит от GitHub и от неё же обновляются
установленные клиенты. Файлы берутся из `.local/windows-releases`.
...
- Кинозал ходит к площадкам напрямую, если в `.env` не заданы `CINEMA_PROXY`/`CINEMA_PROXY_<ID>`
  (выход через ваш прокси) или `CINEMA_PRIVATE_HOSTS_<ID>` (частный адрес — одной площадке). Всё
  это, плюс `CINEMA_PROVIDERS` (какие площадки включены) и плеер страниц «По ссылке»
  (`sniffer`, отдельный контейнер за своей сетевой стеной) — со значениями по умолчанию и полным
  списком переменных в [deployment.md](deployment.md#кинозал-выход-наружу-площадок).
