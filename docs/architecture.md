...
якоря, каждый считает сам по `serverTime` снимка. Пультом владеет тот, кто открыл (и ведущий);
сменить или закрыть может любой, кому разрешены интеграции. Активная интеграция одна: музыка и
кинозал друг друга исключают, и это проверяют оба — ядро и служба.
