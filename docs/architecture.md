...
Девятизначные коды назначаются криптографическим генератором, уникальность закреплена индексом PostgreSQL. Миграция V4 и обработчик запуска назначают коды существующим комнатам. `POST /api/v1/rooms/join-by-code` всегда создаёт ожидающий запрос и ограничен по частоте; код не заменяет разрешение на чтение истории, скачивание файлов или публикацию медиа. Ожидающий видит только свою запись. Организатор подтверждает запрос обычной командой допуска.
...
| `PUT /api/v1/favorites/{roomId}` | Сохранить по действующему `roomCredential`; максимум пять, проверка допуска |
