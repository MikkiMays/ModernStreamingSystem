...
Само видео идёт **через сервер**: `services` достаёт у площадки HLS-мастер и проксирует
плейлисты и сегменты со своего адреса, а браузер играет обычный `<video>`. Из браузера к
YouTube и Twitch не уходит ни одного запроса — это и делает кинозал работающим там, где до
площадок не достучаться. Подробности и границы — [INTEGRATIONS.md](INTEGRATIONS.md).
