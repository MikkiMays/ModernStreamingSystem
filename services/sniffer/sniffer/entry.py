...
заменяет себя (`execve`) на tini с коротким окружением, где секрета нет: после `execve` INTERNAL_SECRET нет
ни в памяти процессов контейнера, ни в `/proc/1/environ`. Поэтому в compose у контейнера нет `init: true` —
tini запускается отсюда.

ЧЕГО ЭТО НЕ ПРЯЧЕТ. Сам производный ключ (`CINEMA_SNIFFER_KEY`) лежит в окружении tini и сервера, и
`/proc/1/environ` и `/proc/<pid>/environ` читает любой процесс того же пользователя — браузер тоже, если
чужая страница взломала его рендерер. Этим ключом открывается только `/sniff` самого плеера страниц: ни
подписей кинозала, ни ядра, ни сейфа он не открывает.
"""
