...
`127.0.0.1/32:8097` для домашнего сервера видео, `[fd00::/8]:8096`, `media.lan:8096`. Общей строки на
...
# Соединений в пуле клиента — как у httpx по умолчанию, и у площадок каталога, и у «По ссылке». Готовый файл
# держит одно соединение на зрителя весь фильм (`<video>` читает его одним запросом `Range`), поэтому число
# соединений — это и потолок зрителей таких файлов на весь сервер. Память под чужими чтениями держит не пул,
# а свои пределы: чтение в память (`facade.FOREIGN_READS`) и очередь переписывания; у «По ссылке» меньше
# только простаивающих соединений — сокетов к чужим серверам зря не держим.
POOL = httpx.Limits(max_connections=100, max_keepalive_connections=20)
FOREIGN_POOL = httpx.Limits(max_connections=100, max_keepalive_connections=8)
...
class GuardedTransport(httpx.AsyncHTTPTransport):
    ...
    любыми хостами (`strict`: «По ссылке») — нет: её цель проверяется здесь, до
