...
class HostPolicy:
    ...
    Пока подписанные адреса проверяются общим списком `transport.signer.ALLOWED_HOSTS`; здесь
    тот же список, разложенный по владельцам, — чтобы проверка могла переехать на площадку, не
    потеряв и не добавив ни одного хоста.
    """
