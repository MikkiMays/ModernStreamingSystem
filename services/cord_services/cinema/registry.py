...
# Сколько ждать ответ каталога площадки (её JSON: поиск, разделы, полки, токен входа, ответ плеера). Отвечает
# она за доли секунды; шестьдесят секунд чтения у клиента площадки — это для тел видео. Зависший вопрос
# каталога держал бы всю страницу сцены: полка «Сериалы и шоу» Rutube так и висела минуту над готовой лентой
# эфиров. Через десять секунд это «площадка не ответила», а необязательная полка — просто пустая.
...
class HostPolicy:
    ...
    suffixes: tuple[str, ...] = ()
    # Любой хост, но только публичный («По ссылке»). Публичность проверяется при
